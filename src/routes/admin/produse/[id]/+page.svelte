...
<div class="page">
  ...
  {#if loading}
    <div class="panel">Se încarcă produsul…</div>
  {:else}
    <form class="panel formGrid" on:submit={submit}>
      <label>
        <span>SKU</span>
        <input class="form-control" bind:value={form.sku} />
      </label>
      <label>
        <span>Nume</span>
      ...
      <div class="full actions">
        <button class="btn btn-primary" type="submit" disabled={saving}>{saving ? 'Se salvează…' : 'Salvează modificările'}</button>
      </div>
    </form>
  {/if}
</div>
